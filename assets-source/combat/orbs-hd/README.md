...
Each `256 × 256` transparent master ships unchanged; Phaser displays it at `42px`
...
- 4프레임 레이어는 `256 × 256` 프레임 네 장을 가로로 붙인 `1024 × 256` 소스를 런타임에서도 그대로 사용한다.
- 정적 레이어도 `256 × 256` 소스를 그대로 사용하며 화면상 크기만 Phaser가 조절한다.
